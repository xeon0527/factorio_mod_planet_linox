...
local power_converter = require("scripts.drv.linox.power-converter")
...
bootstrap.create_tick_handler(function()
  ...
  if surface then
    local reactive_power = surface.find_entity("linox-entity_reactive-power", {0,0})
    if not reactive_power or reactive_power.force.name ~= "player" then return end

    local enable_pwm  = power_converter.get_signal(power_converter.input_type.enable_pwm);
    local turn_on     = power_converter.get_signal(power_converter.input_type.turn_on);

    inductor.enable(enable_pwm > 0.0 and reactive_power);
    if turn_on > 0 then
    ...
    reactive_power.power_usage = math.abs(current_tick_consume - inductor_tick_output);

    power_converter.set_signal(power_converter.output_type.current, 1, inductor.current);
    power_converter.set_signal(power_converter.output_type.target_current, 2, math.floor(current_tick_consume * 60.0 / voltage));
